...
void glfwGetWindowPos(GLFWwindow, int* xpos, int* ypos);
void glfwSetWindowPos(GLFWwindow, int xpos, int ypos);
...
void glfwGetScrollOffset(GLFWwindow window, int* xoffset, int* yoffset);
...
setmetatable(_M, { __index = function(tab, key)
   ...
   if v >= 65 and v <= 90 then
      ...
      s = glfw["glfw" .. first:upper() .. key:sub(2)]
   end
   rawset(tab, key, s)
   return s
end })
